class Array(object):
    ...
    def swap(self, j, k):  # Swap the values at 2 indices
        if (0 <= j < self.__nItems and 0 <= k < self.__nItems):  # bounds check
    ...
    def insertionSort(self):  # Sort by repeated inserts
        for outer in range(1, self.__nItems):  # Mark one element
            ...
            while inner > 0 and temp < self.__a[inner - 1]:  # If marked
                self.__a[inner] = self.__a[inner - 1]  # elem smaller
                inner -= 1  # shift elem to right
            self.__a[inner] = temp  # Move marked elem to 'hole'
            

    def median(self):  # Method for calculating the median
        if self.__nItems == 0:  # Check if the array is empty
            return None

        # Create a sorted copy of the current elements
        sorted_array = sorted(self.__a[:self.__nItems])  # Sort only the valid elements
        mid = self.__nItems // 2  # Calculate the middle position

        # Return the median based on the number of elements
        if self.__nItems % 2 == 1:  # If odd, return the middle element
            return sorted_array[mid]
        else:  # If even, return the average of the two central elements
